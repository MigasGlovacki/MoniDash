...
string(FIND "${SNAPSHOT_TEXT}" "for (const auto& key : keys)" CANDIDATE_LOOP_POSITION)
string(FIND "${SNAPSHOT_TEXT}" "const auto directory_count" CANDIDATE_LOOP_END_POSITION)
if(CANDIDATE_LOOP_POSITION LESS 0 OR CANDIDATE_LOOP_END_POSITION LESS 0 OR CANDIDATE_LOOP_END_POSITION LESS CANDIDATE_LOOP_POSITION)
    message(FATAL_ERROR "exit snapshot candidate loop is missing")
endif()
math(EXPR CANDIDATE_LOOP_LENGTH "${CANDIDATE_LOOP_END_POSITION} - ${CANDIDATE_LOOP_POSITION}")
string(SUBSTRING "${SNAPSHOT_TEXT}" ${CANDIDATE_LOOP_POSITION} ${CANDIDATE_LOOP_LENGTH} CANDIDATE_LOOP_TEXT)
if(NOT CANDIDATE_LOOP_TEXT MATCHES "std::errc::no_such_file_or_directory")
    message(FATAL_ERROR "missing snapshot candidates must have an explicit nonfatal branch")
endif()
if(NOT CANDIDATE_LOOP_TEXT MATCHES "continue")
    message(FATAL_ERROR "missing snapshot candidates must continue probing")
endif()
if(CANDIDATE_LOOP_TEXT MATCHES "else if[ ]*\\([ ]*error[ ]*\\)[^{]*\\{[^}]*return[ ]*;")
    message(FATAL_ERROR "candidate probing must not abort on every filesystem error")
endif()
